...
object Actions {
  ...
  private def checkForInvalidBid(game: Game, bid: Int): Either[String, Unit] = {
    ...
    else if (bid >= 0 && bid <= game.highestBid) Left(s"Bid must be greater than bid form previous bidder (${game.highestBid})")
    else Right()
  }
  ...
  def makeBid(game: Game, bid: Int): Either[String, Game] = {
    ...
    for {
      ...
      _ <- if (playerGamePoints < 1000) Right() else Left(s"${game.activePlayerIndex} are not allowed to bid if total points above 1000. Only pass allowed.")
  ...
  def takeCards(game: Game): Either[String, Game] = {
    if (game.phase != TakeCards) Left("Cannot take cards. No take-cards phase now.")
      ...
      getPlayer(game, game.activePlayerIndex).flatMap(player => {
        ...
        // Info msg about what happened this turn
        val msg = s"ACTION: ${game.activePlayerIndex} took ${game.cardsToTake.foldLeft("")((acc, cur) => acc + cur.toString + " ")} from the board"
        println(msg)
        println()
  ...
  def passCards(game: Game, cardToLeft: Card, cardToRight: Card): Either[String, Game] = {
    for {
      ...
      // Info msg about what happened this turn
      val msg: String = s"ACTION: ${game.activePlayerIndex} passed $cardToRight to ${game.activePlayerIndex.previous} and $cardToLeft to ${game.activePlayerIndex.next}"
      println(msg)
      println()
      ...
      game.requiredSuit match {
        ...
        case Some(suit) => {
          ...
          if(newCardsOnBoard.length >= 3) {
            ...
            val newPlayers = game.players.updated(game.activePlayerIndex, newActivePlayer).updated(playerIndexTakingTrick, newPlayerTakingTrick)

            // change phase if last card played
            val newPhase = if(newActivePlayer.cards.isEmpty) game.phase.nextPhase else game.phase

            // Info msg about what happened this turn
            val msg: String = s"ACTION: ${game.activePlayerIndex} played $card. ${playerIndexTakingTrick} took down a trick of " +
              s"${newCardsOnBoard.foldLeft("")((acc, cur) => acc + cur.toString + " ")}and collected $pointsFromTrick points"
            println(msg)
            println()

            game.copy(players = newPlayers, phase = newPhase, cardsOnBoard = Nil, requiredSuit = None, activePlayerIndex = playerIndexTakingTrick)
            ...
            game.copy(cardsOnBoard = newCardsOnBoard, activePlayerIndex = game.activePlayerIndex.next, players = newPlayers)
          }
        }
        ...
        case None => {
          ...
          val newPlayers = game.players.updated(game.activePlayerIndex, newActivePlayer).updated(game.activePlayerIndex, newActivePlayer.copy(points = newActivePlayer.points + pointsFromMarriage))
          ...
          game.copy(cardsOnBoard = newCardsOnBoard, requiredSuit = Some(card.suit), trump = newTrump, activePlayerIndex = game.activePlayerIndex.next, players = newPlayers)
        }
      }
    }
  }
  ...
  def updateGameAfterRound(game: Game): Either[String, Game] = {
    ...
    def determineRoundPoints(playerIndex: PlayerIndex): Int = {
      ...
      if(playerIndexWinningBid == playerIndex) {
        ...
        val resultPoints = if(diffOfFive > 2) points - diffOfFive + 5
    ...
    val newPointsRound: Map[PlayerIndex, Int] = Map(FirstPlayer -> firstPlayerRoundPoints, SecondPlayer -> secondPlayerRoundPoints, ThirdPlayer -> thirdPlayerRoundPoints)

    val newPointsGame: Map[PlayerIndex, Int] = game.results match {
      case Nil => Map(FirstPlayer -> (500 - firstPlayerRoundPoints), SecondPlayer -> (500 - secondPlayerRoundPoints), ThirdPlayer -> (500 - thirdPlayerRoundPoints))
      case _ =>
        ...
        Map(FirstPlayer -> newFirstPlayerPoints, SecondPlayer -> newSecondPlayerPoints, ThirdPlayer -> newThirdPlayerPoints)
    }
    ...
    val msg = playerIndexWinningBid match {
      case FirstPlayer => s"${FirstPlayer} played a game (${winningBid}) and ${if(firstPlayerRoundPoints > 0) "won" else "lost"}"
      case SecondPlayer => s"${SecondPlayer} played a game (${winningBid}) and ${if(secondPlayerRoundPoints > 0) "won" else "lost"}"
      case ThirdPlayer => s"${ThirdPlayer} played a game (${winningBid}) and ${if(thirdPlayerRoundPoints > 0) "won" else "lost"}"
    }
    println(msg)
    println()
    ...
    determineWinnerOfGame match {
      case None => Right(Game.init(game.roundNumber + 1, game.playerIndexStartingThisRound.next, newResults, Deck.shuffle))
